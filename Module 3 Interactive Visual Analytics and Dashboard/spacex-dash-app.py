...
launch_sites_options.extend([{'label': i, 'value': i} for i in spacex_df['Launch Site'].unique()])
...
@app.callback(
    ...
    Input('spacex-launch-dropdown', 'value'),
)
...
@app.callback(
    ...
    Input('spacex-launch-range-slider', 'value'),
    Input('spacex-launch-dropdown', 'value'),
)
def update_scatter_chart(payload_range, selected_site):
    ...
    filtered_df = filtered_data[mask]
    fig = px.scatter(
        filtered_df,
        x='Payload Mass (kg)',
        y='class',
